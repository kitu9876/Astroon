    ...
    ERC2981Upgradeable
{
    ...
    function startPreSale(
        ...
        SaleDetailMap[saleId] = SaleInfo(_cost, _mintCost, _maxSupply, _startTime, _endTime, _maxSupply);
    ...
    function updateCategory(CATEGORY[] memory _category, uint256[] memory _id) external onlyOwner {
    ...
    function validateNftLimit(address _addr, uint256 nftQty) internal view {
        ...
        require(nftBalance + nftQty <= maxPresaleLimit, "buying Limit exceeded");

        uint256 count = spendAmount >= tierMap[1].minValue && spendAmount < tierMap[1].maxValue
            ? 2
            : spendAmount >= tierMap[2].minValue && spendAmount < tierMap[2].maxValue
            ? 4
            : spendAmount >= tierMap[3].minValue && spendAmount < tierMap[3].maxValue
            ? 6
            : spendAmount >= tierMap[4].minValue && spendAmount < tierMap[4].maxValue
            ? 8
            : 10;
        require(count >= nftBalance && (count - nftBalance) >= nftQty, "buying Limit exceeded");
    }
    ...
    function buyPresale(uint256 nftQty) external payable {
        require(
            SaleDetailMap[saleId].startTime <= block.timestamp && SaleDetailMap[saleId].endTime >= block.timestamp,
        ...
        require(tokenIdCount.current() + nftQty <= SaleDetailMap[saleId].maxSupply, "Not enough tokens");
        ...
        token.transferFrom(msg.sender, owner(), nftQty * (SaleDetailMap[saleId].cost));
        payable(owner()).transfer(msg.value);
        emit BoughtNFT(_msgSender(), nftQty, saleId);
    }

    function minting(CATEGORY[] memory _category) external onlyOwner {
    ...
    function _beforeTokenTransfer(
        ...
        if ((rewardEnable && from != address(0)) && checkTokenRewardEligibility(tokenId)) {
            uint256 _rewards = astRewards.getRewardsCalc(tokenId);
...
//0x91754f2873EAc59B4908024F3B3Ec6a4400B8be1   - Proxy
//0xfffB28b1cb66D8B149a90991b0B8db81A4aEB67e - Implementation
