...
contract ASTTokenRewards is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, DateTime {
    ...
    function checkReward(address _addr) external view returns (uint256) {
        UserTokenDetails storage userDetails = userTokenDetailsMap[_addr];

        uint256 rewards;

        uint256 currMonth = DateTime.getMonth(block.timestamp);
        uint256 nftBalance = nftContract.balanceOf(_addr);
        for (uint256 i; i < nftBalance; i++) {
            uint256 id = nftContract.tokenOfOwnerByIndex(_addr, i);
            bool IsEligible = nftContract.checkTokenRewardEligibility(id);
            if (IsEligible) {
                uint256 amount = getRewardsCalc(id);
        ...
        uint256 prev = singleMonthClaims[_addr][currMonth];
    ...
    function allowedWithdraw() internal view returns (uint256) {
        uint8 currMonth = DateTime.getMonth(block.timestamp);
    ...
    function updateLimits(uint256[] memory _limits) external {
        require(_limits.length == 12, "Invalid Length");
        for (uint256 i = 0; i < 12; i++) {
            WithdrawlMap[i + 1] = _limits[i];
        }
    }
}
// AST Rewards :
//  0xB832be6E9231444C00B1D2BE70883E2654F61f3c  proxy  same as earlier
//  0xD837A1926609a84086c374D495178f96a7932e95  implmentn
