// AST NFT test cases with deducting AST token
...
describe("Unit Tests", function () {
  ...
  beforeEach(async function () {
    ...
    const nft = await ethers.getContractFactory("ASTNFTnew");
  ...
  describe("ASTNFT", () => {
    it("PreSale buy one by one  with AST token deduction", async function () {
      ...
      await token.connect(user).approve(astNft.address, (150 * 10 ** 18).toString());
      var tx = await astNft.connect(user).buyPresale(1, { value: (1 * (0.1 * 10 ** 18)).toString() });
      //var tx = await token.transferFrom(user.address, astNft.address, 150 * 10 ** 18);
      var txn = await tx.wait();
      ...
      var tx = await astNft.connect(user).balanceOf(user.address);
